...
lasso <- function(x,y,above=0) {
	...
	x <- as.matrix(x)
	fit.cv <- cv.glmnet(x=x,y=y,family="gaussian",alpha=1,standardize=FALSE,nfolds=5) #data have been already standardized 
        
	lambda <- fit.cv$lambda.min
	final <- glmnet(x=x,y=y,family="gaussian",alpha=1,lambda=lambda, standardize=FALSE) # glmnet is standardized by default, so turn it off
